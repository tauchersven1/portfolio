...
public class PortfolioSnapshot
{
    public static PortfolioSnapshot create(Portfolio portfolio, CurrencyConverter converter, LocalDate date)
    {
        List<SecurityPosition> positions = portfolio.getTransactions().stream()
                        ...
                        .collect(Collectors.groupingBy(PortfolioTransaction::getSecurity)).entrySet().stream().map(e -> {
                            SecurityPrice price = e.getKey().getSecurityPrice(date);

                            if (price.getValue() == 0L)
                            {
                                List<PortfolioTransaction> tx = e.getValue();
                                Optional<PortfolioTransaction> last = tx.stream().sorted(Transaction.BY_DATE.reversed())
                                                .findFirst();
                        ...
                        }).filter(p -> p.getShares() != 0).collect(Collectors.toList());
    ...
    {
        return positions.stream().map(position -> position.calculateValue(date))
