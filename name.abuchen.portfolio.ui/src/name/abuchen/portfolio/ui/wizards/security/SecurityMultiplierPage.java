...
import name.abuchen.portfolio.model.InteractiveBrokersOptionSymbol;
...
public class SecurityMultiplierPage extends AbstractPage
{
    ...
    public void createControl(Composite parent)
    {
        ...
        loadDerivativeData();
        applyInteractiveBrokersOptionDefaults();
    ...
    private void createDerivativeMasterData(Composite container)
    {
        ...
        lastTradingDay.addSelectionListener(new SelectionAdapter()
        {
            @Override
            public void widgetSelected(SelectionEvent e)
            {
                copyLastTradingDayToRelatedDates();
            }
        });
    ...
    private void createMultiplierSection(Composite container)
    {
        ...
        group.setText("Multiplier history");
        ...
        Composite editor = new Composite(group, SWT.NONE);
        ...
        setMultiplierDate(defaultEffectiveDate());
        ...
        Button add = new Button(editor, SWT.PUSH);
        add.setText("Add / replace");
    ...
    private void createDeltaSection(Composite container)
    {
        ...
        setDeltaDate(defaultEffectiveDate());
    ...
    private TableViewer createHistoryViewer(Composite parent, String valueTitle, int type)
    {
        TableViewer result = new TableViewer(parent, SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE);
        ...
        GridDataFactory.fillDefaults().grab(true, true).hint(275, 170).applyTo(result.getControl());
        TableViewerColumn dateColumn = new TableViewerColumn(result, SWT.NONE);
        ...
        TableViewerColumn valueColumn = new TableViewerColumn(result, SWT.RIGHT);
    ...
    private void createActions(Composite parent, TableViewer table, List<?> values, String title, String message)
    {
    ...
    private void applyInteractiveBrokersOptionDefaults()
    {
        InteractiveBrokersOptionSymbol.parse(security.getTickerSymbol()).ifPresent(parsed -> {
            if (derivativeType.getSelectionIndex() == 0)
                derivativeType.select(2);
            if (putCall.getSelectionIndex() == 0)
                selectByValue(putCall, parsed.putCall(), "CALL", "PUT");
            if (strike.getText().isBlank())
                strike.setText(parsed.strike().toPlainString());
            if (contractSymbol.getText().isBlank())
                contractSymbol.setText(parsed.tradingSymbol());
            if (underlying.getText().isBlank())
                underlying.setText(parsed.tradingSymbol());
            if (lastTradingDay.getValue() == null)
            {
                lastTradingDay.setDate(parsed.expirationDate());
                copyLastTradingDayToRelatedDates();
            }
            else
            {
                if (expirationDate.getValue() == null)
                    expirationDate.setDate(parsed.expirationDate());
                if (settlementDate.getValue() == null)
                    settlementDate.setDate(parsed.expirationDate());
            }
        });
    }

    private void copyLastTradingDayToRelatedDates()
    {
        LocalDate date = lastTradingDay.getDate();
        if (date == null)
            return;
        expirationDate.setDate(date);
        settlementDate.setDate(date);
    }

    private LocalDate defaultEffectiveDate()
    {
        return client.getPortfolios().stream().flatMap(p -> p.getTransactions().stream())
                        .filter(t -> t.getSecurity() == security && t.getDateTime() != null)
                        .map(t -> t.getDateTime().toLocalDate()).max(LocalDate::compareTo).orElse(LocalDate.now());
    }
    ...
    private static final class OptionalDateField
    {
        ...
        private void addSelectionListener(SelectionAdapter listener)
        {
            enabled.addSelectionListener(listener);
        ...
        {
            date.setDate(value.getYear(), value.getMonthValue() - 1, value.getDayOfMonth());
